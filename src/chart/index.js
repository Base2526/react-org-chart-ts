...
import { collapse, wrapText, helpers } from '../utils';
import defineBoxShadow from '../defs/defineBoxShadow';
import defineAvatarClip from '../defs/defineAvatarClip';
...
export default function init(options) {
  ...
  if (!treeData.children) {
    console.error('react-org-chart: missing children')
    return
  }
  ...
  const minZoom = (config && config.minZoom) ? config.minZoom : 0.1;
  const maxZoom = (config && config.maxZoom) ? config.maxZoom : 2;
  // Defined zoom behavior
  var zoom = d3.behavior
    .zoom()
    .scaleExtent([minZoom, maxZoom]) // mini zoom, max zoom
    .duration(50)
    .on('zoom', zoomed)
  ...
  function resetPosition(){
    const TranslateX = parseInt(childrenWidth + (elemWidth - childrenWidth * 2) / 2 - margin.left / 2);
    const TranslateY = 48;
  ...
  d3.select(`#${resetPositionId}`).on('click', ()=>resetPosition())
